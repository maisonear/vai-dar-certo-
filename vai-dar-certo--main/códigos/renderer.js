...
class Renderer {
    constructor(canvas) {
        ...
        this.assets = {
            player:  new Image(),
            playerWalkRight: new Image(),
            playerWalkLeft: new Image(),
            playerWalkUp: new Image(),
            playerWalkDown: new Image(),
            ...
            // Goblin
            goblinWalkRight: new Image(),
            goblinWalkLeft: new Image(),
            goblinAttack: new Image(),
            ...
            bossRight: new Image(),
            bossLeft: new Image(),
            bossAttackRight: new Image(),
            bossAttackLeft: new Image()
        };
        this.assetsLoaded = 0;
        this.totalAssets  = 18;
    ...
    loadAssets() {
        // Detect current depth to handle both root and códigos/ entry points
        // Using a more robust check for folder name in the URL
        const isSubfolder = /[\/\\]c[oó]digos[\/\\]/i.test(window.location.pathname) || 
                           window.location.pathname.endsWith('index.html');
        const prefix = isSubfolder ? '../' : '';

        this.assets.player.src  = prefix + 'entidades/player.png';
        this.assets.playerWalkRight.src = prefix + 'protagonista/andandodireitaprotagonista-removebg-preview.png';
        this.assets.playerWalkLeft.src  = prefix + 'protagonista/andandoesquerdaprotagonista-removebg-preview.png';
        this.assets.playerWalkUp.src    = prefix + 'protagonista/cimaprotagonista-removebg-preview.png';
        this.assets.playerWalkDown.src  = prefix + 'protagonista/baixoprotagonista-removebg-preview.png';
        this.assets.playerAttack.src    = prefix + 'protagonista/ataqueprotagonista-removebg-preview.png';
        this.assets.enemies.src = prefix + 'entidades/enemies.png';
        this.assets.icons.src   = '';
        this.assets.tileset.src = prefix + 'cenários/tileset.png';
        ...
        const onError = (name) => () => {
            this.assetsLoaded++; // Count as loaded to avoid blocking logic, though it failed
        ...
        this.assets.playerWalkRight.onload = onLoad('playerWalkRight');
        this.assets.playerWalkLeft.onload  = onLoad('playerWalkLeft');
        this.assets.playerWalkUp.onload    = onLoad('playerWalkUp');
        this.assets.playerWalkDown.onload  = onLoad('playerWalkDown');
        this.assets.playerAttack.onload    = onLoad('playerAttack');

        // Goblin
        this.assets.goblinWalkRight.onload = onLoad('goblinWalkRight');
        this.assets.goblinWalkLeft.onload  = onLoad('goblinWalkLeft');
        this.assets.goblinAttack.onload    = onLoad('goblinAttack');

        // Boss
        this.assets.bossUp.onload          = onLoad('bossUp');
        this.assets.bossDown.onload        = onLoad('bossDown');
        this.assets.bossRight.onload       = onLoad('bossRight');
        this.assets.bossLeft.onload        = onLoad('bossLeft');
        this.assets.bossAttackRight.onload = onLoad('bossAttackRight');
        this.assets.bossAttackLeft.onload  = onLoad('bossAttackLeft');
        ...
        this.assets.goblinWalkRight.src = prefix + 'entidades/goblin/inimigogoblinandando-removebg-preview.png';
        this.assets.goblinWalkLeft.src  = prefix + 'entidades/goblin/inimigogoblinandandoesquerda-removebg-preview.png';
        this.assets.goblinAttack.src    = prefix + 'entidades/goblin/inimigogoblinatacandodireita-removebg-preview.png';

        this.assets.bossUp.src          = prefix + 'entidades/boss/boss cima.png';
        this.assets.bossDown.src        = prefix + 'entidades/boss/boss baixo.png';
        this.assets.bossRight.src       = prefix + 'entidades/boss/boss direita.png';
        this.assets.bossLeft.src        = prefix + 'entidades/boss/boss esquerda.png';
        this.assets.bossAttackRight.src = prefix + 'entidades/boss/boss ataque direita.png';
        this.assets.bossAttackLeft.src  = prefix + 'entidades/boss/boss ataque esquerda.png';
    ...
    drawPlayer(player) {
        ...
        // Determinar qual spritesheet usar
        let currentSheet = this.assets.playerWalkDown;
        let numFrames = 3;

        // PRIORIDADE: Movimento deve sobrescrever idle/cooldown, mas o ataque ativo tem prioridade
        if (player.isAttacking) {
            currentSheet = this.assets.playerAttack;
            numFrames = 1;
        } else if (player.velocityX !== 0 || player.velocityY !== 0) {
            switch(player.lastMoveDir) {
                case 'right': currentSheet = this.assets.playerWalkRight; numFrames = 5; break;
                case 'left':  currentSheet = this.assets.playerWalkLeft;  numFrames = 5; break;
                case 'up':    currentSheet = this.assets.playerWalkUp;    numFrames = 3; break;
                case 'down':  currentSheet = this.assets.playerWalkDown;  numFrames = 3; break;
            }
        } else {
        ...
        if (currentSheet && currentSheet.complete && currentSheet.naturalWidth > 0) {
            ...
            // Calculate Frame
            const frameIndex = player.animFrame % numFrames;
            const fw = currentSheet.naturalWidth / numFrames;
            const fh = currentSheet.naturalHeight;

            ctx.translate(player.x, player.y);

            const displaySize = 128; // User specified 128x128
            ctx.drawImage(
                currentSheet,
                frameIndex * fw, 0, fw, fh,
                -displaySize / 2, -displaySize / 2 - 20,
    ...
    drawProjectile(proj) {
        ...
        if (proj.type === 'chaos_ball') {
            ...
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.arc(-2, -2, proj.radius * 0.4, 0, Math.PI * 2);
            ctx.fill();
        } else {
    ...
    drawEnemy(enemy) {
        ...
        // 1. Shadow for all enemies
        ctx.fillStyle = 'rgba(0,0,0,0.3)';
        ctx.beginPath();
        ctx.ellipse(enemy.x, enemy.y + 15, 15, 8, 0, 0, Math.PI * 2);
        ctx.fill();

        // 2. Determine Spritesheet and Frames
        let currentSheet = null;
        let numFrames = 3;
        let isAnimated = false;
        let mirrorX = false;
        let displaySize = 64; // Default screen size

        if (enemy.type === 'goblin') {
            isAnimated = true;
            displaySize = 128; // User size
            if (enemy.isAttacking) {
                currentSheet = this.assets.goblinAttack;
                numFrames = 2;
                mirrorX = (enemy.facing === 'left');
            } else {
                currentSheet = (enemy.facing === 'left') ? this.assets.goblinWalkLeft : this.assets.goblinWalkRight;
                numFrames = 3;
            }
        } else if (enemy.type === 'harbinger_of_chaos' || enemy.type === 'boss') {
            isAnimated = true;
            displaySize = 160; // Boss size
            if (enemy.isAttacking) {
                currentSheet = (enemy.facing === 'left') ? this.assets.bossAttackLeft : this.assets.bossAttackRight;
                numFrames = 2;
            } else {
                numFrames = 3;
                switch(enemy.lastDir) {
                    case 'up':    currentSheet = this.assets.bossUp; break;
                    case 'down':  currentSheet = this.assets.bossDown; break;
                    case 'left':  currentSheet = this.assets.bossLeft; break;
                    case 'right': currentSheet = this.assets.bossRight; break;
                }
            }
        }

        // 3. Render
        if (isAnimated && currentSheet && currentSheet.complete && currentSheet.naturalWidth > 0) {
            const frameIndex = enemy.animationFrame % numFrames;
            const fw = currentSheet.naturalWidth / numFrames;
            const fh = currentSheet.naturalHeight;
            ...
            ctx.drawImage(
                currentSheet,
                frameIndex * fw, 0, fw, fh,
                -displaySize / 2, -displaySize / 2 - 10,
                displaySize, displaySize
            );
        } else {
            // Original vector fallback for non-animated enemies
            const colors = { skeleton: '#e5e5e5', wolf: '#475569', golem: '#94a3b8' };
            const sizes  = { skeleton: 14, wolf: 16, golem: 22 };
        ...
        if (!enemy.isDead) {
            ...
            ctx.textAlign  = 'center';
            const names = { goblin: 'Goblin', harbinger_of_chaos: 'Harbinger of Chaos' };
            ctx.fillText(names[enemy.type] || enemy.type, enemy.x, enemy.y - yOffset - 10);
        }
