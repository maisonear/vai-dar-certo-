...
class Player {
    constructor() {
        ...
        this.animFrame = 0;
        this.animTimer = 0;
        this.animSpeed = 0.12; // segundos por frame
    ...
    useItem(itemId) {
        ...
        // Reset stats
        // Reset to 5 in each base stat
        this.statPoints += (Object.values(this.baseStats).reduce((a, b) => a + b, 0) - (11 * 5));
    ...
    update(deltaTime) {
        ...
        if (this.velocityX !== 0 || this.velocityY !== 0) {
            ...
            if (Math.abs(this.velocityX) > Math.abs(this.velocityY)) {
                this.lastMoveDir = this.velocityX > 0 ? 'right' : 'left';
            } else {
                this.lastMoveDir = this.velocityY > 0 ? 'down' : 'up';
            }
            ...
            if (this.animTimer >= this.animSpeed) {
                ...
                // Max frames depend on direction (handled in renderer, but we cycle here)
                // We'll use a common denominator or handle overflow in renderer
                if (this.animFrame >= 10) this.animFrame = 0; 
            }
            }
        } else {
            // Idle state
        ...
        // Reset attack state visually if moving or timer expires
        if (this.isAttacking) {
            this.attackAnimTimer += deltaTime;
            if (this.attackAnimTimer > 0.4 || (this.velocityX !== 0 || this.velocityY !== 0)) {
                this.isAttacking = false;
                this.attackAnimTimer = 0;
            }
        }
        ...
        // MP Regen based on senses/int
        const mpRegen = this.getStat('inteligência') * 0.1;
        this.currentMP = Math.min(this.getMaxMP(), this.currentMP + mpRegen * deltaTime);
