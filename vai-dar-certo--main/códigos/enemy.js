...
class Enemy {
    ...
    update(deltaTime, player) {
        ...
        // Reset attack state visually
        if (this.isAttacking) {
            this.attackAnimTimer += deltaTime;
            if (this.attackAnimTimer >= 0.4) {
                this.isAttacking = false;
                this.attackAnimTimer = 0;
                
                // Specific Boss Behavior: Spawn projectile on attack finish
                if (this.type === 'harbinger_of_chaos' && window.game) {
        ...
        if (distance < this.aggroRange) {
            if (distance > this.attackRange && !this.isAttacking) {
                ...
                if (this.attackCooldown <= 0) {
                    ...
                    this.attackCooldown = (this.type === 'harbinger_of_chaos') ? 2.5 : 1.5; 
                }
            }
        }
        ...
        if (this.animationTimer >= this.animSpeed) {
            this.animationTimer = 0;
            this.animationFrame++;
            
            // Frame caps
            let maxFrames = 3;
            if (this.type === 'goblin') maxFrames = 3;
            if (this.isAttacking) maxFrames = 2;
