...
class UISystem {
    ...
    updateHotbar(player) {
        ...
        equippedSkills.forEach(skill => {
            ...
            // Skill icon slot with ID for highlighting
            const slotDiv = document.createElement('div');
            slotDiv.className = 'hotbar-slot';
            slotDiv.id = `hotbar-${skill.key}`;
            slotDiv.innerHTML = `
        ...
        // 4. Render Potion slots (Fixed)
        const potionSlots = [
            { key: 'q', icon: '🧪', id: 'hotbar-potion-weak' },
            { key: 'l', icon: '💧', id: 'hotbar-potion-mp' },
            { key: 'k', icon: '🍯', id: 'hotbar-potion-strong' }
        ];

        potionSlots.forEach(p => {
            const itemDiv = document.createElement('div');
            itemDiv.className = 'hotbar-item potion-item';
            
            const slotDiv = document.createElement('div');
            slotDiv.className = 'hotbar-slot';
            slotDiv.id = p.id;
            slotDiv.innerHTML = `
                <div class="key-hint">${p.key.toUpperCase()}</div>
